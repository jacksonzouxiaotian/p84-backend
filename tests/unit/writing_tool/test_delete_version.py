def test_delete_version_success(auth_client, mock_s3_client, test_user, db):
    """Test deleting a specific document version."""
    ...
    res = auth_client.delete(f"/writing_tool/documents/{doc.id}/versions/v1.0")
    assert res.status_code == 200
    assert res.get_json()["code"] == 0

def test_delete_nonexistent_version(auth_client):
    response = auth_client.delete('/writing_tool/documents/1/versions/v9.9')
